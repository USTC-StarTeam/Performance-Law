...
def train_epoch(args,epoch, device, model_name,toprint_epoch):
    l2_emb = 0.0
    start_time=time.time()
    print(len(train_loader))
    for step, (user_arr, seq_arr, pos_arr, neg_arr) in tqdm(enumerate(train_loader)):
        # load data to device
        ...
        if toprint_epoch:
            if step % log_interval == 0:
                ...
                logger.info("#####################iter")
                t_test = evaluate(model.module, dataset, maxlen, device)
                # 提取指标值
                NDCG200, NDCG50, NDCG10, HT200, HT50, HT10, MRR = t_test

                # 打印结果
                logger.info('test (NDCG@200: %.4f, NDCG@50: %.4f, NDCG@10: %.4f, HR@200: %.4f, HR@50: %.4f, HR@10: %.4f, MRR: %.4f)' % 
                    (NDCG200, NDCG50, NDCG10, HT200, HT50, HT10, MRR))
                print(args)
                print('test (NDCG@200: %.4f, NDCG@50: %.4f, NDCG@10: %.4f, HR@200: %.4f, HR@50: %.4f, HR@10: %.4f, MRR: %.4f)' % 
                    (NDCG200, NDCG50, NDCG10, HT200, HT50, HT10, MRR))
...
if __name__=="__main__":
    ...
    parser.add_argument('--out_dir', default='/root/autodl-tmp/model/sasrec8', type=str)
    ...
    eval_interval = 1
    eval_every_epoch=1
    log_interval = 3000*96/args.batch_size
    save_interval = 10000
    eval_iters = 200
   ...
   # various inits, derived attributes, I/O setup
    ...
    if not eval_only:
        ...
        for epoch in range(max_epoch):
            if eval_only: break
            if epoch_cnt%eval_every_epoch==0 and epoch_cnt!=0:
                train_epoch(args,epoch, device, model_name,True)
            else:
                train_epoch(args,epoch, device, model_name,False)
            if epoch_cnt%eval_every_epoch==0 and epoch_cnt!=0:
                if ddp:
                    ...
                    torch.save(raw_model.state_dict(),'{}/{}_epoch_{}.pth'.format(save_dir,model_name,epoch))
                    print("#####################epo")
                    t_test = evaluate(raw_model, dataset, maxlen, device)
                    # 提取指标值
                    NDCG200, NDCG50, NDCG10, HT200, HT50, HT10, MRR = t_test
